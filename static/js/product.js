...
HU.Showcase = function(){
    this.elements = {};
    this.elements.container = $('.gallery-showcase');
...
HU.OfferList = function(){
    this.elements = {};
    this.elements.container = $('.offer-container');
    this.elements.fromSelect = this.elements.container.find('#from-select');
    this.elements.dailySelect = this.elements.container.find('#daily-select');
    this.elements.list = this.elements.container.find('.offer-list');
    this.elements.items = this.elements.list.find('.item');

    this.binds();
};

HU.OfferList.prototype = {
    binds: function(){
        var that = this;
    ...
    makeOriginList: function(list){
        ...
        list.map(function(index, item){
            return $(item).find('.origin-list .origin-item');
        }).map(function(index, origin){
            var tempArry = [];
            var furfles = origin.map(function(index, item){
                tempArry.push(slugifier(item.innerText));
                allOrigins.push(item.innerText);
            });
            var flat = tempArry.join('|');
            return flat;
        }).map(function(index, item){
            $(list[index]).attr('data-origins',item);
        });

        allOrigins.filter(unique)
        .sort()
...
function slugifier(val) {
  function furfles(string) {
      if (!string) return '';
